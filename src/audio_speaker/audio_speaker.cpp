...
int HBAudioSpeaker::Run() {
  if (!is_init_) {
    RCLCPP_ERROR(rclcpp::get_logger("audio_speaker"), "HBAudioSpeaker not init.");
    return -1;
  }

  rclcpp::executors::SingleThreadedExecutor exec;
  auto capture_task = std::make_shared<std::thread>(
      std::bind(&HBAudioSpeaker::SpeakerWriteThread, this));
  exec.spin();
...
int HBAudioSpeaker::SpeakerWriteThread() {
  ...
  while (rclcpp::ok()) {
    // ret = alsa_device_read(uac_device_, buffer, frames);
    // if (ret <= 0) continue;
    // RCLCPP_DEBUG(rclcpp::get_logger("audio_speaker"), "uac get audio size:%d",
    //              size);
...
int HBAudioSpeaker::ParseConfig(std::string config_file) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("audio_speaker"), "hobot speaker config file:%s",
              config_file.c_str());
  std::ifstream ifs(config_file);
  if (!ifs.is_open()) {
    RCLCPP_ERROR(rclcpp::get_logger("audio_speaker"), "open config file:%s fail",
                 config_file.c_str());
    return -1;
  }
