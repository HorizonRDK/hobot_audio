...
int AudioEngine::ProcessData(char* in_data,
  ...
  if(out_len < hrsc_out_buffer.size) {
    RCLCPP_WARN(rclcpp::get_logger("audio_capture"),
                "out data realloc, %d, %d!", out_len, hrsc_out_buffer.size);
    delete[] out_data;
    out_data = new char[hrsc_out_buffer.size];
    memset(out_data, 0, hrsc_out_buffer.size);
  }
  memcpy(out_data, hrsc_out_buffer.audio_data, hrsc_out_buffer.size);
  out_len = hrsc_out_buffer.size;
  return ret;
}
